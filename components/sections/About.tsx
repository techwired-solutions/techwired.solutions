      ...
      <Container className="flex flex-col gap-14">
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
          <Reveal className="flex flex-col gap-4">
            ...
            <h2 className="tnt text-[clamp(28px,5vw,52px)]">
              A small team that{" "}
          ...
          <Reveal delay={0.08} className="flex flex-col gap-5">
            ...
            <p className="text-[14px] text-ink/45">Founded by {site.founder}.</p>
          </Reveal>
        </div>

        <Reveal className="grid grid-cols-1 gap-px overflow-hidden rounded-card border border-black/10 bg-black/10 sm:grid-cols-3">
          {facts.map((f) => (
            <div key={f.v} className="flex flex-col gap-1 bg-whiteout p-6">
              <span className="tnt text-[28px]">{f.k}</span>
              <span className="text-[13px] uppercase tracking-[0.14em] text-ink/45">
                {f.v}
              </span>
            </div>
          ))}
